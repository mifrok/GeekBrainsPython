...
class Cell:
    ...
    def __sub__(self, other):
        if self.count - other <= 0:
            print('Нельзя отнять {}, текущее количество {}'.format(other, self.count))
        else:
    ...
    def make_order(self, row):
        try:
            result = (('*' * row + '\n')  # Составляем макет строки
                      * (self.count // row)  # Считаем количество строк
                      + '*' * (self.count % row))  # Добавляем остаток в последнюю строку
        except ZeroDivisionError:
            print('В ряду не может быть 0 знаков!')
            return
        return result


new_cell = Cell(20)
print(new_cell.count)
new_cell + 20
print(new_cell.count)
new_cell - 10
print(new_cell.count)
